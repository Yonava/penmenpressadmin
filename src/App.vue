<template>
  <div class="form-container">
    ...
    <div class="cat-container">
      ...
      photo
      <input v-model="newArticle.photo" type="text" placeholder="image" />
      category
      <input v-model="newArticle.category" type="text" placeholder="category" />
      content
      ...
      <div style="margin: 10px">
        ...
        <select v-model="newArticle.issueId">
          <option v-for="issue in issues" :value="issue.id" :key="issue.id">
            {{ issue.releaseMonth }}/{{ issue.releaseDay }}/{{
              issue.releaseYear
            }}
          </option>
        </select>
      ...
      <div>
        <div v-for="article in articles" :key="article.id">
          ...
          <button @click="deleteArticle(article.id)" style="margin-left: 10px">
            x
          </button>
        </div>
      </div>
      ...
      <div>
        <div v-for="article in articles" :key="article.id">
          <span>{{ article.title }}</span>
          <button @click="editArticle(article)" style="margin-left: 10px">
            edit
          </button>
        </div>
      </div>
      <div v-if="Object.keys(selectedArticle).length">
        <h3>Editing: {{ selectedArticle.title }}</h3>
        <span>
          Add Authors:
          <button @click="addAuthorToArticle">+</button>
        </span>
        <div v-for="(author, index) in articleAuthors" :key="author.id">
    ...
    <div class="cat-container">
      ...
      <input
        ...
        type="text"
      />
      <input v-model="newAuthor.lastName" placeholder="last name" type="text" />
      <input v-model="newAuthor.title" placeholder="title" type="text" />
      <textarea v-model="newAuthor.bio" placeholder="bio"></textarea>
      <input v-model="newAuthor.photo" placeholder="image url" type="text" />
      <input v-model="newAuthor.joinDay" placeholder="join day" type="number" />
      <input
      ...
      <div>
        <div v-for="author in authors" :key="author.id">
          ...
          <button @click="deleteAuthor(author.id)" style="margin-left: 10px">
            x
          </button>
        </div>
      </div>
      ...
      <div>
        <div v-for="author in authors" :key="author.id">
          <span>{{ author.firstName }} {{ author.lastName }}</span>
          <button @click="editAuthor(author)" style="margin-left: 10px">
            edit
          </button>
        </div>
        <div v-if="Object.keys(selectedAuthor).length">
          ...
          <input
            ...
            type="text"
          />
          <textarea v-model="selectedAuthor.bio" placeholder="bio"></textarea>
          <input
    ...
    <div class="cat-container">
      <h3>Add Issue:</h3>
      <input v-model="newIssue.photo" placeholder="photo" type="text" />
      <input
      ...
      <input
        ...
        type="number"
      />
      <button @click="addIssue">add</button>
      <h3>Delete Issue:</h3>
      <div>
        <div v-for="issue in issues" :key="issue.id">
          ...
          >
          <button @click="deleteIssue(issue.id)" style="margin-left: 10px">
            x
          </button>
        </div>
      </div>
      ...
      <div>
        <div v-for="issue in issues" :key="issue.id">
          ...
          >
          <button @click="editIssue(issue)" style="margin-left: 10px">
            edit
          </button>
        </div>
...
export default {
  ...
  mounted() {
    axios
      .get("https://press-app.herokuapp.com/api/articles")
      .then((response) => {
    ...
    axios
      .get("https://press-app.herokuapp.com/api/issues")
      .then((response) => {
    ...
    axios
      .get("https://press-app.herokuapp.com/api/authors")
      .then((response) => {
  ...
  methods: {
    addAuthor() {
      ...
      axios
        .post("https://press-app.herokuapp.com/api/authors", this.newAuthor)
        .then(() => {
          location.reload();
        })
        .catch((error) => {
    ...
    deleteAuthor(authorId) {
      axios
        .delete(`https://press-app.herokuapp.com/api/authors/${authorId}`)
        .then(() => {
          location.reload();
        })
        .catch((error) => {
    ...
    saveAuthor() {
      axios
        .put(
          `https://press-app.herokuapp.com/api/authors/${this.selectedAuthor.id}`,
          this.selectedAuthor
        )
        .then(() => {
          location.reload();
    ...
    addIssue() {
      axios
        .post("https://press-app.herokuapp.com/api/issues", this.newIssue)
        .then(() => {
          location.reload();
        })
        .catch((error) => {
    ...
    deleteIssue(issueId) {
      axios
        .delete(`https://press-app.herokuapp.com/api/issues/${issueId}`)
        .then(() => {
          location.reload();
        })
        .catch((error) => {
    ...
    saveIssue() {
      axios
        .put(
          `https://press-app.herokuapp.com/api/issues/${this.selectedIssue.id}`,
          this.selectedIssue
        )
        .then(() => {
          location.reload();
    ...
    addArticle() {
      ...
      axios
        .post("https://press-app.herokuapp.com/api/articles", this.newArticle)
        .then(() => {
          location.reload();
        })
        .catch((error) => {
    ...
    deleteArticle(articleId) {
      axios
        .delete(`https://press-app.herokuapp.com/api/articles/${articleId}`)
        .then(() => {
          location.reload();
        })
        .catch((error) => {
    ...
    async saveArticle() {
      await axios
        .delete(
          `https://press-app.herokuapp.com/api/articleAuthor/${this.selectedArticle.id}`
        )
        .catch((error) => {
      ...
      this.articleAuthors.forEach(async (articleAuthor) => {
        await axios
          .post(
            `https://press-app.herokuapp.com/api/articleAuthor/${this.selectedArticle.id}/${articleAuthor}`
          )
          .catch((error) => {
            console.log(error);
          });
      ...
      axios
        .put(
          `https://press-app.herokuapp.com/api/articles/${this.selectedArticle.id}`,
